...
function loadPage() {
	$('#genres-btn').click(paintButtons);
}

function paintButtons(e) {
	var $button;
	var containButtons = $('#container-btn');
	...
	request.get(api_endpoint_genres).then(function (response) {//response es de donde vamos a extraer información
		...
		genresMovie.forEach(element => {
			...
			$button = $('<input>').attr('type', 'button').attr('value', movieName).attr('data-movies', genresId).addClass('button-gnr text-white pointer');
...
function paintTitleMovies(event) {
	...
	var containerTitles = $('#container-btn'); //traer contenedor de peliculas
	...
	request.get(url_list_genres).then(function (response) {
		...
		movies.forEach(element => {
			...
			titles = $('<h4 />').text(movieTitle).attr('data-movie', movieId).addClass('title text-white row pointer');
...
function paintInfoMovie(event) {
	...
	request.get(api_endpoint_info).then(function (response) {
	  ...
	  var $imgPoster = $("<img>").attr("src", moviePoster);
		var $title =  $("<h4 />").text(movieTitle).addClass("text-yellow");
		var $tagline = $("<h5 />").text(movieTagline).addClass("text-white");
		var $overview = $("<p />").text(movieOverview).addClass("text-white");
		var $date = $("<p />").text(movieDate).addClass("text-white");
		var $page = $("<a />").attr("href",  moviePage).text("Homepage").addClass("text-yellow");
		// var $movies = $("<button />").text("movies").addClass("this-movies");

		infoMovie.append($imgPoster);
		infoMovie.append($title);
		infoMovie.append($tagline);
		infoMovie.append($overview);
		infoMovie.append($date);
		infoMovie.append($page);
		// infoMovie.append($movies);

		// $movies.click(paintTitleMovies);
		//   crear elementos
		// agregar contenido
